var ManageManage = (function () {
	return {
		...
		query: function(){
			...
			var deadline=$("#manage-search").find("#manage-deadline").combobox('getValue');
		 ...
		 */
		importData : function(){
			var importDialog = $("<div id='manage-import'></div>").dialog({
			    title: '数据导入',
			    width: 400,
			    height: 160,
			    closed: false,
			    cache: false,
			    resizable : false,
			    content: '<form method="POST" enctype="multipart/form-data" action="imp-manage/import/manage"><table style="margin-top:10px;"><tr style="height:30px;"><td style="width:30%;text-align: center;">管理费清单:</td><td><input id="upfile" name="upfile" type="file"/></td></tr></table></form>',
			    modal: true,
			    buttons : [{
					iconCls: "icon-save",
					text : '导入',
		                	...
		                	importDialog.find('form').ajaxSubmit({    
		                            ...
		                            success: function(manageImportResult){
		                            	...
		                            	if(manageImportResult && manageImportResult.length > 0){
		                            		if(manageImportResult.indexOf("导航菜单") > 0){
