...
public class ImpController {
	...
	@RequestMapping(value = "/import/{name}", method = { RequestMethod.GET,
			RequestMethod.POST })
	@ResponseBody
	public String importExcel(@PathVariable("name") String name,
			...
			try {
				if ("insure".equalsIgnoreCase(name)) {
					...
					try {
						...
						result.append(getCellValue(row.getCell(2)).trim())
								.append(",\n");
					}
		...
		if (null != carNum && !carNum.isEmpty()) {
			...
			if (null != endDateStr && !endDateStr.isEmpty()) {
				if (endDateStr.contains(".")) {
					endDateStr = endDateStr.substring(0,
							endDateStr.indexOf("."));
				}
			...
			Gps gps = null;
			Car car = carService
					.findByCarNum("陕" + carNum.trim().toUpperCase());
			if (null != car) {
